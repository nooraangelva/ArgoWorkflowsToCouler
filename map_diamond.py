...
def map_diamond():
    '''main function (the brains) creates dependencies and call functions '''
    ...
    couler.set_dependencies(lambda: echo_trice(
        lista_a, "A"), dependencies=None)
    couler.set_dependencies(lambda: echo_trice(
        lista_a, "B"), dependencies=["A"])
    couler.set_dependencies(lambda: echo_trice(
        lista_a, "C"), dependencies=["A"])
    couler.set_dependencies(lambda: echo_trice(
        lista_a, "D"), dependencies=["B", "C"])
